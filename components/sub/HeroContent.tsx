    ...
    <InView triggerOnce={false}>
      {({ inView, ref }) => (
        <motion.div
          ...
          className="flex md:flex-row flex-col items-center justify-center gap-10 md:gap-0 md:px-20 px-5 mt-40 w-full z-20"
        >
          <div className="h-full w-full md:w-3/6 flex flex-col gap-5 justify-center text-start">
            <div className="hidden md:flex flex-row items-center md:gap-5 gap-1">
              {heroContent.badges.map((badge) => (
                <InView key={badge} triggerOnce={false}>
                  {({ inView, ref }) => (
                    <motion.div
                      ref={ref}
                      initial="hidden"
            ...
            <InView triggerOnce={false}>
              {({ inView, ref }) => (
                ...
                >
                  <span>
                    {heroContent.headline.beforeAccent}{" "}
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-cyan-500">
                      {heroContent.headline.accent}{" "}
                    </span>
                    {heroContent.headline.afterAccent}
                  </span>
                </motion.div>
              )}
            </InView>
          ...
          <InView triggerOnce={false}>
            {({ inView, ref }) => (
              ...
              >
                <Image
                  src={site.assets.heroIllustration}
                  alt={heroContent.media.illustrationAlt}
                  height={650}
                  width={650}
                />
              </motion.div>
